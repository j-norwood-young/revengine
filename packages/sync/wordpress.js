...
async function sync_user(wordpress_user_id) {
    try {
        const api_response = (await axios.get(`${config.wordpress.revengine_api}/user/${wordpress_user_id}`, { headers: { Authorization: `Bearer ${process.env.WORDPRESS_KEY}` }}));
        console.log(api_response);
        const wpuser = api_response.data.data.pop();
